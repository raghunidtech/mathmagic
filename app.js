...
try {
    ...
    document.body.innerHTML = `<div style="font-family: monospace; color: red; padding: 20px;"><h1>FIREBASE CONFIG ERROR</h1><p>${e.message}</p></div>`;
}
...
const subTopics = {
    ...
    trivia: [
        ...
        { id: 'history_indian', name: 'INDIAN HISTORY' },
...
const topicSelectSP = document.getElementById('topicSelectSP');
...
const topicSelectMP = document.getElementById('topicSelectMP');
...
function handleTimeout() {
    if (screens.result.style.display === 'block' || screens.gameOver.style.display === 'block') return;
    if (gameMode === 'multiplayer' && submitAnswerButton.disabled) return;
    const playerAnswer = (currentQuestion?.questionType === 'n') ? "" : selectedMCAnswer;
    const feedback = `Time's up! The answer was ${currentQuestion?.answer ?? 'N/A'}.`;
    if (gameMode === 'singleplayer') {
        const playerPoints = currentSettings?.penalty || -10;
        const playerFeedback = `Time's up! You lose ${Math.abs(playerPoints)} points. The answer was ${currentQuestion?.answer ?? 'N/A'}.`;
        ...
        bot1Score += bot1Result.isCorrect ? (currentSettings?.points || 10) : (currentSettings?.penalty || -10);
        bot2Score += bot2Result.isCorrect ? (currentSettings?.points || 10) : (currentSettings?.penalty || -10);
        ...
        if (!userId) return;
        processMultiplayerAnswer(playerAnswer, false, feedback);
    }
}
...
function handleSubmit() {
    ...
    if (gameMode === 'singleplayer') {
        const playerPoints = isCorrect ? (currentSettings?.points || 10) : (currentSettings?.penalty || -10);
...
function applyTheme(themeName) {
    ...
    document.body.classList.remove('bg-orange-200', 'text-gray-800');
...
async function initApp() {
    ...
    topicSelectSP.addEventListener('change', () => populateSubTopics('SP'));
    singlePlayerStartButton.addEventListener('click', startSinglePlayerGame);
    topicSelectMP.addEventListener('change', () => populateSubTopics('MP'));
...
function populateSubTopics(modePrefix) {
    ...
    const subs = subTopics[selectedTopic];
...
function startSinglePlayerGame() {
    console.log("[DEBUG SP] Starting single player game...");
    playerScore = 0; bot1Score = 0; bot2Score = 0; questionsAnswered = 0;
    usedQuestionIndicesSP = {};
    currentTopic = topicSelectSP.value;
    currentSubTopic = subTopicSelectSP.value;
    console.log(`[DEBUG SP] Selected topic: ${currentTopic}, branch: ${currentSubTopic}`);
    if (!usedQuestionIndicesSP[currentSubTopic]) {
        console.log(`[DEBUG SP] Initializing used questions structure for ${currentSubTopic} in startSinglePlayerGame`);
...
function loadSinglePlayerQuestion() {
    console.log("[DEBUG SP] Loading single player question...");
    if (questionsAnswered >= TOTAL_QUESTIONS_PER_GAME) { endSinglePlayerGame(); return; } const roll = Math.floor(Math.random() * 6) + 1; let difficulty = difficultyRollMap[roll]; console.log(`[DEBUG SP] Rolled ${roll} -> difficulty ${difficulty}`);
    let bank = questionBank[currentSubTopic]?.[difficulty]; if (!bank || bank.length === 0) {
        console.log(`[DEBUG SP] No questions for ${currentSubTopic}/${difficulty}, falling back to easy.`);
...
async function createGame() {
    ...
    const newGame = { hostId: userId, status: "lobby", public: isPublic, players: { [userId]: { score: 0, name: userDisplayName } }, createdAt: serverTimestamp(), usedQuestionIndices: {} };
    ...
    console.log(`[DEBUG] Listening ID: ${id}`); if (gameUnsubscribe) { gameUnsubscribe(); gameUnsubscribe = null; } const gameRef = doc(db, gameCollectionPath, id); gameUnsubscribe = onSnapshot(gameRef, (docSnap) => {
        ...
        switch (gameData.status) {
            ...
            case "playing": 
                scoreHeader.classList.remove('hidden'); 
                timerArea.classList.remove('hidden'); 
                questionCount.textContent = `Q: ${questionsAnswered + 1}/${TOTAL_QUESTIONS_PER_GAME}`; 
                displayQuestion(gameData); 
                const alreadyAnswered = (gameData.answeredBy || []).includes(userId); 
                submitAnswerButton.disabled = alreadyAnswered; 
                submitAnswerButton.classList.toggle('btn-gray', alreadyAnswered); 
                submitAnswerButton.classList.toggle('btn-yellow', !alreadyAnswered); 
                waitMessage.classList.toggle('hidden', !alreadyAnswered); 
            ...
            case "results": 
                ...
                displayMultiplayerResults(gameData); 
                latestGameData = gameData; 
                showScreen('result'); 
                break;
            case "finished": 
                scoreHeader.classList.remove('hidden'); 
                timerArea.classList.add('hidden'); 
                displayMultiplayerGameOver(gameData); 
                latestGameData = gameData; 
                showScreen('gameOver'); 
                break;
        }
...
function updateMultiplayerScoreDisplay(players) {
...
async function hostLoadQuestion(questionIndex) {
    ...
    currentTopic = currentGameData.topic;
    currentSubTopic = currentGameData.subTopic;
...
async function processMultiplayerAnswer(answer, isCorrect, feedback) {
    ...
    submitAnswerButton.classList.add('btn-gray');
    submitAnswerButton.classList.remove('btn-yellow');
    waitMessage.classList.remove('hidden');

    const points = isCorrect ? (currentSettings?.points || 10) : (currentSettings?.penalty || -10);
    const answerPayload = { answer: answer, isCorrect: isCorrect, feedback: feedback, points: points };
    ...
    try {
        ...
        // Re-enable button if transaction fails
        submitAnswerButton.disabled = false;
        submitAnswerButton.classList.remove('btn-gray');
        submitAnswerButton.classList.add('btn-yellow');
...
function displayMultiplayerGameOver(gameData) {
...
function selectMCAnswer(selectedButton) {
...
function simulateBotAnswer(question, accuracy) {
    const getsItRight = Math.random() < accuracy; if (getsItRight) return { answer: question.answer, isCorrect: true }; if (!question || typeof question.answer === 'undefined') return { answer: "???", isCorrect: false };
